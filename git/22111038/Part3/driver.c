...
static ssize_t my_read(struct file *filp, char *ubuf, size_t length, loff_t * offset){ 
    ...
    if(oper == 0){
		...
		unsigned long req = current->nvcsw;
		sprintf(d_buf, "%ld", req);
	}
                                   ...
                                   const char *buf, size_t count)
{
	...
	//search for current tgid in linked list, if it is not present then only create node else not.	
